...
import { FILTER_OPERATORS } from "../schemas.js";
import {
  errorTextResult,
  errorResult,
  jsonResult,
...
async function handleEditAgentConfig(
  ...
  if (contextVariables !== undefined) {
    ...
    try {
      ...
      return errorTextResult("Error: contextVariables must be a valid JSON array string");
    }
...
async function handleUpdateFilters(
  ...
  try {
    ...
    return errorTextResult("Error: filters must be a valid JSON array string");
  }
  ...
  const typedFilters = filters.map((f: any) => ({
    field: f.field,
    operator: f.operator,
    values: f.values?.map((v: any) => ({ value: v, type: "string" })),
  }));
...
async function handleEditPromptTemplate(
  ...
  if (inputMappingsStr !== undefined) {
    ...
    try {
      ...
      return errorTextResult("Error: inputMappings must be a valid JSON array string");
    }
...
For full-config edits or CRUD (add/edit/save/delete/reprocess/get_data), use the \`column\` tool instead.`,
    {
      ...
      contextVariables: z.string().optional().describe("JSON array of context variables (for edit_agent_config)"),
      ...
      // update_filters
      filters: z.string().optional().describe("JSON array of filters (for update_filters)"),
      // reprocess_typed
      ...
      inputMappings: z.string().optional().describe("JSON array of input mappings (for edit_prompt_template)"),
    },
    async (params) => {
